...
Ensure that the [aws-ofi-rccl plugin](https://github.com/aws/aws-ofi-nccl) is installed, since the Beverin cluster uses Slingshot and libfabric is required in order to use RDMA. For versions of ROCm `<7` enroot hooks are available off-the-shelf on the cluster. For ROCm versions `>= 7`, as of now, it must be built from scratch, follow the instructions [here](https://github.com/aws/aws-ofi-nccl) and configure the necessary environment variables accordingly.
...
This will create `libnccl-tuner.so` that can be loaded by RCCL.
