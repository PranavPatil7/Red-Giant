...
def train(args):
  ...
  torch._inductor.config.max_autotune_gemm = True # some gemms are not perfectly tuned by tunableops, with triton alternatives being faster
  torch._inductor.config.max_autotune_pointwise = True # more tuning never hurts
  ...
  with set_default_dtype(model_dtype):
    ...
    logger.info(f"Successful device transfer")
  ...
  if args.profile:
    # pytorch profiler can export traces for PerfettoUI, which is visually and functionally
    # very similar to NVIDIA Nsight Systems
    prof = torch.profiler.profile( 
      activities=[
          torch.profiler.ProfilerActivity.CPU,
          torch.profiler.ProfilerActivity.CUDA  # works for ROCm
      ],
      record_shapes=True,
      with_stack=True,
    )
